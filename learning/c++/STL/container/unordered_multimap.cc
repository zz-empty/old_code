...
void test()
{
    unordered_multimap<int, string> addr = {
        {1, "wuhan"},
        {1, "wuhan"},
        {2, "beijing"},
        {3, "chongqing"},
...
#if 0
    cout << endl;
    cout << "--------------test unordered_multimap::insert--------------" << endl;
    std::pair<unordered_multimap<int,string>::iterator, bool> ret = addr.insert(std::pair<int, string>(9, "henan"));
    if (ret.second) {
        cout << "insert succeed, ";
...
void test_class()
{
    unordered_multimap<string, Point> addr = {
        {"beijing", Point(1, 1)},
        {"shanghai", Point(2, 3)},
